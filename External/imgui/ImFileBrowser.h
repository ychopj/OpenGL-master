...
//                else
	...
	int escIdx = ImGuiKey_Escape;
    //int escIdx = GetIO().KeyMap[ImGuiKey_Escape];
    if(Button(cancel_.c_str()) || closeFlag_ ||
        ((flags_ & ImGuiFileBrowserFlags_CloseOnEsc) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && escIdx >= 0 && IsKeyPressed(ImGuiKey_Escape)))
        CloseCurrentPopup();
