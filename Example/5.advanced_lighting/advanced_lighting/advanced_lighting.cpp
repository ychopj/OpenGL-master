...
int main()
{
    ...
    // -------------------------
    Shader shader(
        FileSystem::getPath("resource_/shader/1.advanced_lighting.vs").c_str(),
        FileSystem::getPath("resource_/shader/1.advanced_lighting.fs").c_str());
    ...
    if (data)
    {
        ...
        glTexParameteri(
            ...
            format == GL_RGBA ? GL_CLAMP_TO_EDGE
                              : GL_REPEAT); // for this tutorial: use GL_CLAMP_TO_EDGE to prevent semi-transparent borders.
